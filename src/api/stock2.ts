...
export const apiAddStockItem = async (params: StockItemParams) => {
    const res = await req(`${base}/stock2/item`, { method: 'POST', body: JSON.stringify(params) });
    if (res?.code === 200) {
...
export const apiAddDividendItem = async (params: DividendItemParams) => {
    const res = await req(`${base}/stock2/dividend/item`, {
        method: 'POST',
...
export const apiGetStockList = async (params?: ListParams) => {
    ...
    const res = await req(`${base}/stock2/list${query}`);
...
export const apiGetDividendList = async (params?: ListParams) => {
    ...
    const query = searchParams.toString() ? `?${searchParams.toString()}` : '';
    const res = await req(`${base}/stock2/dividend/list${query}`);
...
export const apiEditStockItem = async (params: StockItemParams) => {
    const res = await req(`${base}/stock2/item/${params._id}`, {
        method: 'PATCH',
        body: JSON.stringify(params),
    });
    if (res?.code === 200) {
...
export const apiEditDividendItem = async (params: DividendItemParams) => {
    const res = await req(`${base}/stock2/dividend/item/${params._id}`, {
        method: 'PATCH',
        body: JSON.stringify(params),
    });
    if (res?.code === 200) {
...
export const apiDeleteStockItem = async (_id: string) => {
    const res = await req(`${base}/stock2/item/${_id}`, {
        method: 'DELETE',
    });
    if (res?.code === 200) {
...
export const apiDeleteDividendItem = async (_id: string) => {
    const res = await req(`${base}/stock2/dividend/item/${_id}`, {
        method: 'DELETE',
    });
    if (res?.code === 200) {
        SnackbarUtils.success('刪除成功');
